...
def muse_fit_predict(
    ...
    with tf.Session() as sess:
        ...
        sess.run(
            ...
            feed_dict={triplet_lambda: 0, triplet_margin: 0},
        )
        ...
        for epoch in range(n_epochs_init):
            ...
            if epoch % print_epochs == 0:
                L_total, L_reconstruction, L_weight = sess.run(
                    (loss, reconstruction_error, weight_penalty),
                    feed_dict={
                        x: data_train_x,
                        y: data_train_y,
                        z: data_train_z,
                        ref_label_x: np.zeros(
                            data_train_x.shape[0]
                        ),  # no use as triplet_lambda=0
                        ref_label_y: np.zeros(
                            data_train_y.shape[0]
                        ),  # no use as triplet_lambda=0
                        ref_label_z: np.zeros(
                            data_train_z.shape[0]
                        ),  # no use as triplet_lambda=0
                        triplet_lambda: 0,
                        triplet_margin: 0,
                    },
        ...
        latent, reconstruct_x, reconstruct_y, reconstruct_z = sess.run(
        ...
        latent_x, latent_y, latent_z = sess.run(
        ...
        (
            ...
            (joint_latent, x_hat, y_hat, z_hat, encode_x, encode_y, encode_z),
            feed_dict={
                x: data_x,
                y: data_y,
                z: data_z,
